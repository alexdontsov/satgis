...
def import_data(shapefile):
    ...
    for i in range(layer.GetFeatureCount()):
        ...
        print geom_field
        if geom_field == 'geom_line string':
            continue
...
def export_data(shapefile):
    ...
    # shapefile = shapefile.encode('utf-8')
    layer = datasource.CreateLayer(shapefile.filename.encode('utf-8'),
                                   dst_spatial_ref)

    # Define the shapefile's attributes.
    for attr in shapefile.attribute_set.all():
        field = ogr.FieldDefn(attr.name.encode('utf-8'), attr.type)
