...
from app.views import all_waterobjects, one_waterobject_by_slug, one_article_by_slug, getParams
...
urlpatterns = [
    ...
    url(r'^get-params/(?P<slug>[\w-]+)/(?P<param>[\w-]+)/(?P<data_at>[\w-]+)/(?P<data_to>[\w-]+)/$', getParams),
